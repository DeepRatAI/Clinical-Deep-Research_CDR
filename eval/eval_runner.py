...
def load_baseline(baseline_path: str) -> dict[str, Any] | None:
    ...
    if not path.exists():
        print(f"⚠️   Baseline not found: {baseline_path} — skipping comparison")
        return None
    with open(path) as f:
        return json.load(f)
...
def generate_markdown_summary(
    ...
    for r in summary.get("results", []):
        ...
        vc = m.get("verification_coverage", "—")
    ...
    if comparisons:
        ...
        for comp in comparisons:
            ...
            if comp.get("comparison") == "no_baseline":
                lines.append(f"| {qid} | — | — | — | — | No baseline |")
                continue
            for metric, d in comp.get("diffs", {}).items():
                reg = "⚠️ YES" if d.get("regression") else "✅ No"
                lines.append(
                    f"| {qid} | {metric} | {d.get('baseline', '—')} "
                    f"| {d.get('current', '—')} | {d.get('delta', '—')} | {reg} |"
                )
        lines.append("")
...
def run_evaluation(
    ...
    Args:
        ...
        output_format: Output format - 'json' (default) or 'markdown'.
    ...
    for i, q in enumerate(questions, 1):
        ...
        latency = result.get("latency_ms", {}).get("total", "—")
...
Examples:
  ...
  # Combine options:
  python -m eval.eval_runner --mode online --format markdown --output eval/results/
        """,
    )
    parser.add_argument(
        "--dataset",
    ...
    parser.add_argument(
        ...
        default="json",
        dest="output_format",
        help="Output format: 'json' (default) or 'markdown' (human-readable table)",
    )
    parser.add_argument(
        "--compare-baseline",
        default=None,
